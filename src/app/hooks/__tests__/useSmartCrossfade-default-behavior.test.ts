...
describe("Crossfade Default Behavior - Tests", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    jest.useFakeTimers();
    // Clear console logs
  ...
  it("should not trigger crossfade on page load", () => {
    const { result } = renderHook(() =>
      useSmartCrossfade({
        playerStates: mockPlayerStates,
        startCrossfade: mockStartCrossfade,
  ...
  it("should not trigger crossfade on auto-load of track", () => {
    const { result } = renderHook(() =>
      useSmartCrossfade({
        playerStates: mockPlayerStates,
        startCrossfade: mockStartCrossfade,
    ...
    const autoLoadStates = {
      ...
      B: {
        ...mockPlayerStates.B,
        currentTrack: { id: "auto_loaded_track", title: "Auto Loaded Track" },
      },
    };
  ...
  it("should trigger crossfade only after user action (manual trigger)", async () => {
    const { result } = renderHook(() =>
      useSmartCrossfade({
        playerStates: mockPlayerStates,
        startCrossfade: mockStartCrossfade,
  ...
  it("should track previous track IDs with prevTrackRef", () => {
    const { result, rerender } = renderHook(
      ({ playerStates }) =>
        useSmartCrossfade({
          playerStates,
  ...
  it("should track user action with hasUserActionRef", () => {
    const { result, rerender } = renderHook(
      ({ playerStates }) =>
        useSmartCrossfade({
          playerStates,
    ...
    // No user action yet - should skip
    act(() => {
      jest.advanceTimersByTime(1000);
    });
    expect(mockStartCrossfade).not.toHaveBeenCalled();
