...
describe("Crossfade Behavior - Final Implementation", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    jest.useFakeTimers();
  });
  ...
  it("should not trigger crossfade on initial page load or session restore", () => {
    const { result } = renderHook(() =>
      useSmartCrossfade({
        playerStates: mockPlayerStates,
        startCrossfade: mockStartCrossfade,
  ...
  it("should trigger auto-crossfade only after user action (track change)", () => {
  ...
  it("should track track changes correctly for user action detection", () => {
    const { result, rerender } = renderHook(
      ({ playerStates }) =>
        useSmartCrossfade({
          playerStates,
