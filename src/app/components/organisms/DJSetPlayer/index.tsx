...
// Manager instances - imported from separate files
...
class SpotifyPlayerManager {
  ...
  async resumePlayer(playerId: string): Promise<void> {
    ...
    try {
      // Check if there's track info for this player
...
export default function DJSetPlayer({ className = "" }: DJSetPlayerProps) {
  ...
  // Track which players are currently being initialized to prevent duplicate calls
  const initializingPlayers = useRef<Set<string>>(new Set());

  const youtubeManager = useRef(new YouTubePlayerManager());
  const spotifyManager = useRef(new SpotifyPlayerManager());
  ...
  useEffect(() => {
    return () => {
      youtubeManager.current.destroyAll();
      spotifyManager.current.destroyAll();
  ...
  */
  ...
  const initializePlayer = async (
    ...
    if (initializingPlayers.current.has(playerId)) {
      console.log(
        `⏳ Player ${playerId} is already being initialized, skipping...`
      );
      return;
    }

    initializingPlayers.current.add(playerId);

    try {
      console.log(`🎯 Initializing player ${playerId} with service:`, service);
      ...
      if (service === ServiceType.Youtube) {
        ...
        // If container is not found, wait for it to be available
        if (!container) {
          console.log(`⏳ Container for ${playerId} not found, waiting...`);
          // Wait for the container to be available (max 2 seconds)
          let attempts = 0;
          ...
          if (!container) {
            console.error(
              `❌ YouTube player container for ${playerId} not found after waiting!`
            );
            return;
          }
        ...
        await youtubeManager.current.createPlayer(
          playerId,
        ...
        await spotifyManager.current.createPlayer(
          playerId,
          spotifySong.id,
          token
        );
      ...
      // Start progress tracking for this player
      ...
      setTimeout(async () => {
        if (service === ServiceType.Youtube) {
          try {
            const duration =
              youtubeManager.current.getDuration(playerId) * 1000; // Convert to ms
            if (duration > 0) {
              // Initialize the internal state
              youtubeManager.current.setPlayerState(playerId, {
                currentTime: 0,
                duration,
                isPlaying: false,
              });
          ...
          try {
            const player = spotifyManager.current.getPlayer(playerId);
            if (player && typeof player.getCurrentState === "function") {
              const state = await player.getCurrentState();
              if (state && state.duration > 0) {
                // Initialize the internal state
                spotifyManager.current.setPlayerState(playerId, {
                  currentTime: state.position || 0,
      ...
      initializingPlayers.current.delete(playerId);
    }
  };
  ...
  const handleSeek = async (playerId: "A" | "B", position: number) => {
    ...
    if (playerState.service === ServiceType.Youtube) {
      ...
      youtubeManager.current.seekPlayer(playerId, position / 1000); // Convert to seconds for YouTube API
    } else {
  ...
  const startProgressTracking = (playerId: "A" | "B") => {
    ...
    if (players[playerId].service === ServiceType.Spotify) {
      ...
      if (player) {
        ...
        if (!spotifyManager.current.eventListeners) {
          spotifyManager.current.eventListeners = new Map();
        }
        spotifyManager.current.eventListeners.set(playerId, handleStateChange);
      }
    }
    ...
    const timeout = setInterval(async () => {
      ...
      if (playerState.service === ServiceType.Youtube) {
        try {
          const currentTime =
            youtubeManager.current.getCurrentTime(playerId) * 1000; // Convert to ms
          const duration = youtubeManager.current.getDuration(playerId) * 1000; // Convert to ms
          const playerStateNum =
            youtubeManager.current.getPlayerState(playerId);
  ...
  const handlePlay = async (playerId: "A" | "B") => {
    ...
    if (playerState.service === ServiceType.Youtube) {
      ...
      const player = youtubeManager.current.getPlayer(playerId);
      ...
      youtubeManager.current.playPlayer(playerId);
    } else {
      // For Spotify, check authentication first
      const token = localStorage.getItem("spotify_token");
      if (!token) {
      ...
      const trackInfo = spotifyManager.current.trackInfo.get(playerId);
      const player = spotifyManager.current.getPlayer(playerId);

      console.log(`🔍 Spotify play debug for ${playerId}:`, {
      ...
      if (trackInfo && player) {
        // Player exists and track is loaded, just resume
        console.log(`Resuming existing Spotify player ${playerId}`);
        try {
          await spotifyManager.current.resumePlayer(playerId);
        } catch (error) {
          console.error("Spotify resume failed, redirecting to login:", error);
          window.location.href = "/api/spotify/login?origin=/reamp";
          return;
        }
      } else if (trackInfo && !player) {
        // Track info exists but player doesn't, need to create player
        console.log(
          `Creating Spotify player ${playerId} for track ${trackInfo.trackId}`
        );
        try {
          await spotifyManager.current.createPlayer(
            playerId,
          ...
          console.error(
            "Spotify player creation failed, redirecting to login:",
            error
          );
          window.location.href = "/api/spotify/login?origin=/reamp";
          return;
        ...
        // No track info, this shouldn't happen for a dropped song
        console.error(
          `No track info found for player ${playerId}, cannot play`
        );
        return;
      }
    }
    ...
    if (playerState.service === ServiceType.Youtube) {
      youtubeManager.current.setVolume(playerId, intendedVolume);
    } else {
      spotifyManager.current.setVolume(playerId, intendedVolume);
    }
  ...
  const handlePause = async (playerId: "A" | "B") => {
    ...
    if (playerState.service === ServiceType.Youtube) {
      youtubeManager.current.pausePlayer(playerId);
    } else {
      await spotifyManager.current.pausePlayer(playerId);
    }
  ...
  const handleStop = async (playerId: "A" | "B") => {
    ...
    if (playerState.service === ServiceType.Youtube) {
      youtubeManager.current.stopPlayer(playerId);
    } else {
      await spotifyManager.current.pausePlayer(playerId);
    }
  ...
  const handleVolumeChange = (playerId: "A" | "B", volume: number) => {
    ...
    if (playerState.service === ServiceType.Youtube) {
      youtubeManager.current.setVolume(playerId, volume);
    } else {
      spotifyManager.current.setVolume(playerId, volume);
    }
  ...
  const handleMuteToggle = (playerId: "A" | "B") => {
    ...
    if (playerState.service === ServiceType.Youtube) {
      youtubeManager.current.setVolume(playerId, targetVolume);
    } else {
      spotifyManager.current.setVolume(playerId, targetVolume);
    }
  ...
  const handleScratch = (
    ...
    if (playerState.service === ServiceType.Youtube) {
      ...
      youtubeManager.current.seekPlayer(playerId, newTime / 1000); // Convert back to seconds for YouTube API
      ...
      const currentTime = spotifyManager.current.getCurrentTime(playerId);
      const newTime =
        direction === "forward"
          ? Math.min(currentTime + 5000, playerState.duration) // 5 seconds in ms
          : Math.max(currentTime - 5000, 0);
  ...
  const triggerCrossfade = async (endingPlayerId: "A" | "B") => {
    ...
    if (!otherPlayer.isPlaying) {
      ...
      if (otherPlayer.service === ServiceType.Spotify) {
        const player = spotifyManager.current.getPlayer(otherPlayerId);
        if (player && typeof player.seek === "function") {
        ...
        console.log(`🎵 Seeking ${otherPlayerId} to beginning for crossfade`);
        youtubeManager.current.seekPlayer(otherPlayerId, 0);
      }
      ...
      if (otherPlayer.service === ServiceType.Youtube) {
        ...
        try {
          ...
          const playerState =
            youtubeManager.current.getPlayerState(otherPlayerId);
          console.log(
          ...
          youtubeManager.current.playPlayer(otherPlayerId);
        ...
        try {
          const trackInfo = spotifyManager.current.trackInfo.get(otherPlayerId);
          if (trackInfo) {
            await spotifyManager.current.playTrack(
              otherPlayerId,
              trackInfo.trackId
            );
          } else {
            await spotifyManager.current.resumePlayer(otherPlayerId);
          }
        } catch (error) {
          console.error(
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      if (endingPlayer.service === ServiceType.Youtube) {
        youtubeManager.current.setVolume(endingPlayerId, safeEndingVolume);
      } else {
        spotifyManager.current.setVolume(endingPlayerId, safeEndingVolume);
      }

      if (otherPlayer.service === ServiceType.Youtube) {
        youtubeManager.current.setVolume(otherPlayerId, safeOtherVolume);
      } else {
        spotifyManager.current.setVolume(otherPlayerId, safeOtherVolume);
      }
    ...
    if (endingPlayer.service === ServiceType.Youtube) {
      youtubeManager.current.stopPlayer(endingPlayerId);
    } else {
      await spotifyManager.current.pausePlayer(endingPlayerId);
    }
    ...
    if (otherPlayer.service === ServiceType.Youtube) {
      youtubeManager.current.setVolume(otherPlayerId, originalOtherVolume);
      console.log(
        `🎵 Set YouTube volume for ${otherPlayerId} to ${originalOtherVolume}`
      );
    } else {
      spotifyManager.current.setVolume(otherPlayerId, originalOtherVolume);
      console.log(
  ...
  useEffect(() => {
    ...
    // Only run when the song/service/active state changes
  }, [players.A.song, players.A.isActive, players.A.service]);
  ...
  useEffect(() => {
    ...
    // Only run when the song/service/active state changes
  }, [players.B.song, players.B.isActive, players.B.service]);
