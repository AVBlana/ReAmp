...
export default function DJPlayerA({
  playerState,
  onStateChange,
  onClear,
  isActive,
  ...
  useEffect(() => {
    const handleDJPlayerDrop = (event: CustomEvent) => {
      const { playerId, draggableId } = event.detail;
      if (playerId === "A") {
  ...
  useEffect(() => {
    ...
    const player = new window.YT.Player(containerRef.current, {
      ...
      events: {
        onReady: (event: any) => {
          console.log("Player A YouTube ready");
  ...
  }, [isApiReady, playerState.song, playerState.service]);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-bold text-[#FF6B6B]">Player A</h3>
      ...
      <div className="flex-1 relative min-h-0">
        <Droppable droppableId="dj-player-A">
          {(provided, snapshot) => (
            <div
